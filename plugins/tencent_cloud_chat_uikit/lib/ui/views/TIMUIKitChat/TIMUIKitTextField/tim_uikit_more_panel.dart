...
class _MorePanelState extends TIMUIKitState<MorePanel> {
  ...
  _sendVideoMessage(
    ...
    String tempPath =
        "${(await getTemporaryDirectory()).path}${p.basename(originFilePath)}.jpeg";
      ...
      if (PlatformUtils().isMobile) {
        if (PlatformUtils().isAndroid) {
          ...
          if ((androidInfo.version.sdkInt) >= 33) {
            // Android 13+ 走系统 Photo Picker（不依赖 READ_MEDIA_* 广泛权限）
          } else {
  ...
  _sendFile(TUIChatSeparateViewModel model, TUITheme theme) async {
    try {
      final convID = widget.conversationID;
      final convType = widget.conversationType;
      FilePickerResult? result = await FilePicker.platform.pickFiles();
  ...
  _goToVideoUI(String type) async {
    if (!PlatformUtils().isWeb) {
      ...
      if (type == TYPE_VIDEO) {
        hasCameraPermission = await Permissions.checkPermission(
          context,
          Permission.camera.value,
        );
