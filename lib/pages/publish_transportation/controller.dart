...
class PublishTransportationController extends GetxController with ApiMixin {
  static const String _draftKey = 'publish_transportation_draft';
  ...
  List<Category> get categories => ConfigService.to.trafficCategories;
  ...
  final _publish = GuidePublishTransportation().obs;
  GuidePublishTransportation get publish => _publish.value;
  ...
  onSubmit() async {
    ...
    final url = editor == GuidePublishEditor.add
        ? ApiUrl.guideTransportationAdd
        : ApiUrl.guideTransportationEdit;
    final payload = publish.toJson();
    debugPrint('[PublishTransport] POST $url payload keys: ${payload.keys}, name: ${payload['name']}');
    final res = await post(url, data: payload);
    Loading.dismiss();
    if (!res.isSuccess) {
      debugPrint('[PublishTransport] Submit failed: code=${res.code}, message=${res.message}');
...
extension on PublishTransportationController {
  fetchCity() async {
  ...
  fetchInfo() async {
    if (id == 0) return;
    Loading.show();
    final res = await get(ApiUrl.guideTransportationInfo, parameters: {
      'id': id,
    });
    Loading.dismiss();
    if (!res.isSuccess) return;
    _publish.value = GuidePublishTransportation.fromJson(res.dataJson);
    fillData();
  }
