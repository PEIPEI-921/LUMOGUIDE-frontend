...
class PublishAttractionController extends GetxController with ApiMixin {
  static const String _draftKey = 'publish_attraction_draft';
  ...
  List<Category> get categories => ConfigService.to.scenicCategories;
  ...
  final _publish = GuidePublishAttraction().obs;
  GuidePublishAttraction get publish => _publish.value;
  ...
  void onInit() {
    ...
    if (Get.arguments != null) {
      editor =
          Get.arguments['type'] as GuidePublishEditor? ??
  ...
  Map<String, dynamic> _buildDraftMap() {
    _publish.update((val) {
      ...
      val?.startTime = openTimeController.text.trim();
    ...
    return {
      ...
      'tickets_free': publish.ticketsFree,
  ...
  void _fillFromDraft(Map<String, dynamic> draft) {
    ...
    openTimeController.text = draft['start_time'] as String? ?? '';
...
extension on PublishAttractionController {
  fetchCity() async {
    final res = await get(
      ApiUrl.cityList,
  ...
  fetchInfo() async {
    if (id == 0) return;
    Loading.show();
    final res = await get(ApiUrl.guideAttractionInfo, parameters: {'id': id});
    Loading.dismiss();
    if (!res.isSuccess) return;
    _publish.value = GuidePublishAttraction.fromJson(res.dataJson);
    fillData();
  }
