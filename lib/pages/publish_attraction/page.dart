...
class PublishAttractionPage extends StatelessWidget {
  const PublishAttractionPage({super.key});

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(PublishAttractionController());
    return IScaffold(
      appBar: IAppBar(title: '發布景點'.tr),
              ...
              Row(
                children: [
                  Row(
                    ...
                    children: [
                      Icon(
                        controller.publish.ticketsFree == 1
                            ? Icons.check_circle
                            : Icons.circle_outlined,
                        color: controller.publish.ticketsFree == 1
                            ? AppColors.primary
                  ...
                  ).gestures(onTap: () => controller.onToggleFree(true)),
                  20.w.horizontalSpace,
                  Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(
                        controller.publish.ticketsFree == 0
                            ? Icons.check_circle
                            : Icons.circle_outlined,
                        color: controller.publish.ticketsFree == 0
                            ? AppColors.primary
                            : AppColors.assistantText,
                        size: 18.w,
                      ),
                  ...
                  ).gestures(onTap: () => controller.onToggleFree(false)),
                ],
              ),
              if (controller.publish.ticketsFree == 0)
                CustomTextField(
...
class _ImagesWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final controller = Get.find<PublishAttractionController>();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            ...
            Text('景點圖片'.tr).fontSize(14.sp).textColor(AppColors.primaryText),
          ],
        ),
        10.w.verticalSpace,
        Obx(
        ...
        const ImageTipWidget().padding(left: 10.w, vertical: 10.w),
      ],
    );
  }
}
